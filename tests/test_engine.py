...
# ---------------------------------------------------------------------------
# 13. Check extensions: engine finds mate through a check sequence
# ---------------------------------------------------------------------------

def test_check_extension_finds_deeper_mate():
    """Check extensions let the engine find a mate-in-2 through a check at shallow depth.

    Position: White Kd5, Ra2; Black Kb7.
    Mate: 1. Ra7+ Kb8 (Kc8/Kc6 also lose) 2. Ra8#.

    Without check extension at depth=2: after 1. Ra7+ (uses 1 ply), Black replies
    at depth=1. After Kb8, White is at depth=0 → quiescence only sees captures,
    misses the quiet Ra8#.

    With check extension: Ra7+ gives check, so Black's reply is searched at
    depth=2 (extended). After Kb8, White has depth=1 and finds Ra8#.
    """
    board = chess.Board("8/1k6/8/3K4/8/8/R7/8 w - - 0 1")
    tt.clear()
    move = get_best_move(board, depth=2)
    # The only winning plan is Ra7+ followed by Ra8#.
    assert move == chess.Move.from_uci("a2a7"), (
        f"Expected Ra7+ (check leading to mate) but engine played {move.uci()}"
    )
